                                ...
                                <div class="row g-3 mt-3">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select name="division_id" id="division_id" class="form-select"
                                            {{ (isset($user) ? $user->division_id : auth()->user()->division_id) ? 'disabled' : '' }}>
                                            <option value="">{{ __('case.select_division') }}</option>
                                            @foreach ($divisions as $division)
                                                <option value="{{ $division->id }}"
                                                    {{ old('division_id', $user->division_id ?? (auth()->user()->division_id ?? '')) == $division->id ? 'selected' : '' }}>
                                                    {{ app()->getLocale() === 'bn' ? $division->name_bn : $division->name_en }}
                                                </option>
                                            @endforeach
                                        </select>
                                        @php $divisionVal = $user->division_id ?? auth()->user()->division_id; @endphp
                                        @if ($divisionVal)
                                            <input type="hidden" name="division_id" value="{{ $divisionVal }}">
                                        @endif
                                    </div>
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select name="district_id" id="district_id" class="form-select"
                                            {{ (isset($user) ? $user->district_id : auth()->user()->district_id) ? 'disabled' : '' }}>
                                            <option value="">{{ __('case.select_district') }}</option>
                                            @if (isset($user) && $user->division)
                                                @foreach ($user->division->districts as $district)
                                                    <option value="{{ $district->id }}"
                                                        {{ old('district_id', $user->district_id ?? (auth()->user()->district_id ?? '')) == $district->id ? 'selected' : '' }}>
                                                        {{ app()->getLocale() === 'bn' ? $district->name_bn : $district->name_en }}
                                                    </option>
                                                @endforeach
                                            @elseif(auth()->user()->division)
                                                @foreach (auth()->user()->division->districts as $district)
                                                    <option value="{{ $district->id }}"
                                        ...
                                        @php $districtVal = $user->district_id ?? auth()->user()->district_id; @endphp
                                        @if ($districtVal)
                                            <input type="hidden" name="district_id" value="{{ $districtVal }}">
                                        @endif
                                    </div>
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select name="court_id" id="court_id" class="form-select" {{-- Editable in edit mode, locked only if both $user->court_id and !district focal --}}
                                            {{ (isset($user) ? $user->court_id && !$user->district_id : auth()->user()->court_id) ? 'disabled' : '' }}>
                                            <option value="">{{ __('case.select_court') }}</option>
                                            @if (isset($user) && $user->district)
                                                @foreach ($user->district->courts as $court)
                                                    <option value="{{ $court->id }}"
                                                        {{ old('court_id', $user->court_id ?? (auth()->user()->court_id ?? '')) == $court->id ? 'selected' : '' }}>
                                                        {{ app()->getLocale() === 'bn' ? $court->name_bn : $court->name_en }}
                                                    </option>
                                                @endforeach
                                            @elseif(auth()->user()->district)
                                                @foreach (auth()->user()->district->courts as $court)
                                                    <option value="{{ $court->id }}"
                                                        {{ old('court_id', auth()->user()->court_id ?? '') == $court->id ? 'selected' : '' }}>
                                                        {{ app()->getLocale() === 'bn' ? $court->name_bn : $court->name_en }}
                                                    </option>
                                                @endforeach
                                            @endif
                                        </select>
                                        @php $courtVal = $user->court_id ?? auth()->user()->court_id; @endphp
                                        @if ($courtVal)
                                            <input type="hidden" name="court_id" value="{{ $courtVal }}">
                                        @endif
                                    </div>
                                </div>
                                        ...
                                        <div class="card-header d-flex justify-content-between align-items-center">
                                            ...
                                            <div>
                                                <input type="checkbox" class="select-all"
                                                    data-group="{{ $group->id }}"> Select All
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.querySelectorAll('input[name="roles[]"]').forEach(roleCb => {
                    roleCb.addEventListener('change', function() {
                        const selectedRoles = Array.from(document.querySelectorAll(
                            'input[name="roles[]"]:checked')).map(cb => cb.value);

                        // Reset permissions
                        document.querySelectorAll('.permission-checkbox').forEach(cb => cb.checked =
                            false);
                                ...
                                .then(data => {
                                    data.rolePermissions.forEach(permissionId => {
                                        const cb = document.querySelector(
                                            ...
                                            data.permissions.find(p => p.id ==
                                                permissionId).name + '"]');
                                        if (cb) cb.checked = true;
                                    });
                                    document.querySelectorAll('.select-all').forEach(
                                        toggle => {
                                            const groupId = toggle.dataset.group;
                                            updateCount(groupId);
                                        });
